...
import { CalendarIcon, Download, RefreshCw, Database, BarChart3, Table2, Maximize2 } from "lucide-react";
...
// 模拟市场出清概况数据
const generateClearingOverviewData = () => {
  const data = [];
  for (let i = 0; i < 24; i++) {
    const hour = String(i).padStart(2, '0') + ':00';
    data.push({
...
const SpotDisclosure = () => {
  ...
  const supplyDemandData = useMemo(() => generateSupplyDemandData(), []);
  ...
  const clearingOverviewData = useMemo(() => generateClearingOverviewData(), []);
  ...
  const supplyDemandMetrics = useMemo(() => {
    ...
    return {
      ...
      maxLoad: Math.max(...loads).toFixed(0),
      avgBiddingSpace: (supplyDemandData.reduce((sum, d) => sum + d.biddingSpace, 0) / supplyDemandData.length).toFixed(0),
      avgRenewable: (supplyDemandData.reduce((sum, d) => sum + d.renewableOutput, 0) / supplyDemandData.length).toFixed(0),
    };
  }, [supplyDemandData]);
        ...
        <TabsContent value="supply-demand" className="space-y-6">
          ...
          <div className="grid grid-cols-4 gap-4">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold font-mono text-[#00B04D]">{supplyDemandMetrics.avgPrice}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2">
              ...
              <CardContent>
                <div className="text-2xl font-bold font-mono text-blue-500">{supplyDemandMetrics.maxLoad}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2">
              ...
              <CardContent>
                <div className="text-2xl font-bold font-mono text-orange-500">{supplyDemandMetrics.avgBiddingSpace}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2">
              ...
              <CardContent>
                <div className="text-2xl font-bold font-mono text-green-500">{supplyDemandMetrics.avgRenewable}</div>
              </CardContent>
            </Card>
          </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold font-mono text-orange-500">
                  {(clearingOverviewData.reduce((sum, d) => sum + d.interProvincialTie, 0) / clearingOverviewData.length).toFixed(0)}
                </div>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold font-mono text-purple-500">
                  {clearingOverviewData.reduce((sum, d) => sum + d.clearingVolume, 0).toFixed(0)}
                </div>
              </CardContent>
            </Card>
          </div>
